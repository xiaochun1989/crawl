...
class HkexcSpider(CrawlSpider):
    ...
    def get_parse(self, response):
        ...
        for i in content:
            ...
            title = "".join(re.findall('\\\\.*\\\\\w+',i,flags=0))  #join的作用是把列表转为字符串
            title = title.replace('\\\\','\\')
            #检查题目是否包含供股一词
            if '\u4f9b\u80a1' in title:
                title = "u'%s'"%title
                ...
                #列表转为字符串
                item['number'] = "".join(re.findall('hk\d+',i,flags=0))  
                #收录网页地址
                item['url'] = "".join(re.findall('http://stock.*-\d+-\d+',i,flags=0))
                #收录pdf下载地址
                pdfurl = "".join(re.findall('\/\d+.*PDF',i,flags=0))
                item['pdfurl'] = 'http://img2.gtimg.com/finance/hk/pdf' + pdfurl  
                #发布日期
                item['date'] = "".join(re.findall('\d{4}-\d{2}-\d{2}',i,flags=0))
                items.append(item)
        return items
